...
家居场景（参考实验室测试场地照片，scene=True 时生成，纯静态 world geom）：
    ...
    - 门洞处 12mm 木门槛条（真实过坎障碍）；
    - 地面：外间白色亮面砖、里间黑色亮面砖（纯视觉贴片），外间一块 16mm 短毛
      地毯（物理凸台，进出地毯即真实"上毯沿"）；
    - 家具/障碍：细腿餐椅（机器人可从腿间穿过）、木边柜、布艺沙发、白色净化器
      立柱、回充基座+底板、彩色泡沫积木块。
...
def _laser_sites():
    ...
    MuJoCo rangefinder 沿 site 的 +z 轴发射：前向激光 z->+x，左侧激光 z->+y。
    ...
    specs = (
        ...
        ("left",  0.0, CHASSIS_RADIUS, "0.7071068 -0.7071068 0 0"),  # 绕x转-90°: z->+y
    )
    s = []
...
def _scene_geoms():
    ...
    # ---- 地面分区（纯视觉：外间白亮面砖 / 里间黑亮面砖）----
    g.append('    <geom class="visual" name="zone_white" type="box" size="1.59 1.78 0.0008" '
             'pos="0.41 0 0.0008" material="tile_white"/>')
    g.append('    <geom class="visual" name="zone_black" type="box" size="0.59 1.78 0.0008" '
             'pos="2.59 0 0.0008" material="tile_black"/>')
    ...
    # ---- 门洞处 12mm 木门槛条（真实过坎障碍）----
    g.append('    <geom name="door_sill" type="box" size="0.015 0.35 0.006" pos="2.0 0.05 0.006" '
             'material="wood" friction="0.8 0.05 0.05"/>')
    ...
    # ---- 家具 ----
    # 细腿餐椅（里间，机器人可从椅腿之间穿过）
    for i, (lx, ly) in enumerate([(2.55, -1.05), (2.55, -0.75), (2.85, -1.05), (2.85, -0.75)]):
        g.append('    <geom name="chair_leg{i}" type="cylinder" size="0.012 0.21" '
                 'pos="{x} {y} 0.21" material="dark"/>'.format(i=i, x=lx, y=ly))
    g.append('    <geom class="visual" name="chair_seat" type="cylinder" size="0.17 0.02" '
             'pos="2.7 -0.9 0.44" material="wall_white"/>')
    g.append('    <geom class="visual" name="chair_back" type="box" size="0.015 0.14 0.13" '
             'pos="2.86 -0.9 0.59" quat="0.9976 0 0.0698 0" material="wall_white"/>')
    # 木边柜（外间南墙）
...
def build_xml(scene=True):
  ...
  <asset>
    ...
    <material name="tile_black"   rgba="0.07 0.07 0.08 1" reflectance="0.40" specular="0.9" shininess="0.9"/>
