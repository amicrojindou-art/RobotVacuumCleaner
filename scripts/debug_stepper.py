...
# Make the project root importable so `run_experiment` (which lives at the
# repo root) can be found regardless of the current working directory.
...
def lock_walk_mode(env, mode_name, speed):
    """将 JvrcWalkEnv 的步态模式锁定为指定模式。

    mode_name: 'standing' | 'inplace' | 'forward'
    speed: forward 模式下为目标前进速度；standing/inplace 模式下为目标偏航角速度。
    返回是否成功锁定（仅对带 WalkModes 的行走环境有效）。
    """
    task = getattr(env, 'task', None)
    if task is None or not hasattr(task, 'mode'):
        return False
    try:
        from tasks.walking_task import WalkModes
    except Exception:
        return False

    mode_map = {
        'standing': WalkModes.STANDING,
        'inplace': WalkModes.INPLACE,
        'forward': WalkModes.FORWARD,
    }
    if mode_name not in mode_map:
        return False

    task.mode = mode_map[mode_name]
    task.mode_ref = speed
    return True


def run(env, policy, mode=None, speed=0.0):
    observation = env.reset()

    # 锁定步态模式（如果指定了 --mode 且是行走环境）
    locked = False
    if mode is not None:
        locked = lock_walk_mode(env, mode, speed)
        if locked:
            print("已锁定步态模式: {}, 参考值(速度/偏航): {}".format(mode, speed))
            observation = env.get_obs()  # 用锁定后的模式刷新观测
        else:
            print("警告: 当前环境不支持模式锁定, --mode 被忽略")
    ...
    while (ts < end_ts) and (done == False):
        ...
        # 每步重新锁定模式，覆盖 task.step() 内部的随机模式切换
        if locked:
            lock_walk_mode(env, mode, speed)
...
def main():
    ...
    parser.add_argument("--mode",
                        ...
                        type=str,
                        help="锁定步态模式（仅 jvrc_walk 环境有效）：standing/inplace/forward。不指定则使用环境随机模式。",
    )
    parser.add_argument("--speed",
                        default=None,
                        required=False,
                        type=float,
                        help="forward 模式下的目标前进速度(默认0.4)；standing/inplace 模式下为目标偏航角速度(默认0)。",
    )
    ...
    speed = args.speed
    if speed is None:
        speed = 0.4 if args.mode == 'forward' else 0.0
    ...
    run(env, policy, mode=args.mode, speed=speed)
    print("-----------------------------------------")
